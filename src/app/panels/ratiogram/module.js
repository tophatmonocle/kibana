/** @scratch /panels/5
 * include::panels/ratiogram.asciidoc[]
 */

/** @scratch /panels/ratiogram/0
 * == Histogram
 * Status: *Stable*
 *
 * The ratiogram panel allow for the display of time charts. It includes several modes and tranformations
...
function (angular, app, $, _, kbn, moment, timeSeries, numeral) {
  ...
  var module = angular.module('kibana.panels.ratiogram', []);
  app.useModule(module);

  module.controller('ratiogram', function($scope, querySrv, dashboard, filterSrv) {
    $scope.panelMeta = {
      modals : [
        {
          description: "Inspect",
      ...
      editorTabs : [
        {
          title:'Style',
          src:'app/panels/ratiogram/styleEditor.html'
        },
        {
          title:'Queries',
          src:'app/panels/ratiogram/queriesEditor.html'
        },
      ],
      status  : "Stable",
    ...
    $scope.get_data = function(data, segment, query_id) {
      ...
      return results.then(function(results) {
        ...
        if($scope.query_id === query_id) {
          ...
          // finally build a time series of the relative data
          var ratio_ts = new timeSeries.ZeroFilled({
            interval: _interval,
            start_date: _range && _range.from,
            end_date: _range && _range.to,
            fill_style: $scope.panel.derivative ? 'null' : 'minimal'
          ...
          var ratio_data = {};
          var den = 0;
          _.each(data[0].time_series._data, function (val, key) {
            den = data[1].time_series._data[key];
            ratio_data[key] = den === 0 ? 0 : val / den * 100;
          });

          // Tell the ratiogram directive to render.
          data[0].time_series._data = ratio_data;
          $scope.$emit('render', [data[0]]);
  ...
  module.directive('ratiogramChart', function(dashboard, filterSrv) {
    return {
      restrict: 'A',
      template: '<div></div>',
